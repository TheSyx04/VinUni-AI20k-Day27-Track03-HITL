...
async def node_analyze(state):
    ...
    high_risk_terms = ("md5", "sha1", "plaintext", "sql injection", "password", "hard-coded", "hardcoded", "token")
    medium_risk_terms = ("migration", "schema", "database", "sql")
    calibrated_confidence = a.confidence
    ...
    if calibrated_confidence != a.confidence:
        a = a.model_copy(
            update={
                "confidence": calibrated_confidence,
                "confidence_reasoning": (
                    f"{a.confidence_reasoning} "
...
async def node_route(state):
    ...
    logged_decision = "auto" if decision == "auto_approve" else ("escalate" if decision == "escalate" else "pending")
    await audit(state, AuditEntry(
        agent_id=AGENT_ID,
        action="route",
        confidence=c,
...
async def node_human_approval(state):
    ...
    await audit(state, AuditEntry(
        ...
        risk_level=risk_level_for(a.confidence),
        decision="pending",
        reason="Waiting for reviewer approve/reject/edit",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
...
async def node_commit(state):
    ...
    if state.get("escalation_answers"):
        decision = "approve"
    elif state.get("human_choice") in {"approve", "reject", "edit"}:
        decision = state["human_choice"]
    else:
        decision = "reject" if action == "rejected" else "pending"
    await audit(state, AuditEntry(
        agent_id=AGENT_ID,
        action="commit",
...
async def node_escalate(state):
    ...
    await audit(state, AuditEntry(
        ...
        reviewer_id=os.environ.get("GITHUB_USER"),
        decision="escalate",
        reason=f"Received {len(answers)} escalation answers",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
...
async def node_synthesize(state):
    ...
    await audit(state, AuditEntry(
        ...
        reviewer_id=os.environ.get("GITHUB_USER"),
        decision="escalate",
        reason="Refined analysis after escalation Q&A",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
