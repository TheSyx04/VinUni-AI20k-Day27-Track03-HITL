-- Audit trail for the HITL PR-review agent.
--
-- LangGraph's PostgresSaver already persists checkpoints (and lets us resume
...
-- first-class columns (not JSONB) so auditors can query directly:
--
--   SELECT AVG(confidence) FROM audit_events WHERE decision = 'approve';
--   SELECT * FROM audit_events WHERE risk_level = 'high' AND decision != 'reject';
